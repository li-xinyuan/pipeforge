...
3. **Excel/CSV 模板输出** — 贴近业务人员的日常工作方式
...
| **ConfigForge Web** | Vue 3, TypeScript, Naive UI, Pinia, Vite, Vitest | 前端向导 + 配置管理 |
...
| Step 2 | 输入源 | 上传 Excel/CSV 文件或连接数据库 |
| Step 3 | 处理步骤 | SQL 查询 / Python 脚本，支持多步链式 |
| Step 4 | 输出配置 | Excel/CSV 模板、列映射、文件名 |
| Step 5 | 预览导出 | YAML 预览、下载、执行 |
...
| 类型 | 说明 |
|------|------|
| **SQL** | SQLite 查询/过滤/聚合/JOIN/窗口函数，支持多步 DAG 链式执行 |
...
│   ├── config/                 # YAML 配置模型
│   ├── core/                   # 引擎、SQLite 管理器、上下文、注册表
│   └── plugins/                # 输入(excel/csv/database)/处理(sql/python)/输出(excel/csv)
├── configforge/                # ConfigForge API 服务
│   ├── api/                    # REST 路由 (preview/files/ai/wizard/configs/connections)
│   ├── core/                   # 流水线编排 (pipeline)
...
│   ├── src/
│   │   ├── components/         # wizard/step2/step3/step4/ConfettiBurst
│   │   ├── composables/        # useWizardApi/useConfigApi/useFileUpload/useErrorHandler
│   │   ├── stores/             # wizard (Pinia)
│   │   ├── types/              # wizard (TypeScript)
│   │   └── utils/              # serialization/sql
│   └── tests/                  # Vitest (15 files, 113 tests)
...
| Phase 2 — 能力扩展 | 🚧 进行中 | 多步 Pipeline ✅、数据库输入 ✅、AI 编排 ✅、Python 处理器 ✅ |
| Phase 3 — 平台化 | 📋 计划中 | 配置市场、数据血缘、审计日志、Cron 调度、输出格式扩展 |
