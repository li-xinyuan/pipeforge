<template>
  <div class="max-w-3xl mx-auto px-4 py-8">
    <StepIndicator :current-step="5" @goto="(s: number) => { store.goToStep(s); router.push(`/step/${s}`) }" />

    <div class="mb-6">
      <h2 class="text-lg font-semibold text-slate-900 mb-1">预览与导出</h2>
    ...
    <div class="flex justify-between items-center pt-6 border-t border-slate-100 mt-6">
      <button @click="store.prevStep(); router.push('/step/4')" class="text-sm text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-md">&larr; 上一步</button>
...
function refreshPreview() { yamlRef.value?.loadYaml() }
</script>
