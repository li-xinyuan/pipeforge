...
export function useAiGuide() {
  ...
  function buildStep2Prompt(
    ...
    return (
      ...
      `你需要：\n` +
      (hasInputs
        ? `用户已添加了输入源。注意：context 中包含 conversation_history，你必须根据对话历史判断用户刚才说了什么、做了什么选择，**绝对不要重复问已经回答过的问题**。\n` +
          `你需要：\n` +
          `1. 根据对话历史确认用户刚才把哪个文件指定为哪个表\n` +
          `2. 根据场景分析这个输入源应该包含哪些字段/列\n` +
          `3. 告诉用户"好的，**表已确认。根据场景，这个表应包含：XX、XX 等字段"\n` +
          `4. 检查场景还需要哪些输入源，缺几个\n` +
          `5. 如果还缺输入源，引导用户添加下一个："现在来添加**表——它的格式是什么？"\n` +
          `6. 如果输入源数量已满足场景要求，告诉用户"输入源配置完成，可以点下一步了"\n`
