<template>
  <div class="flex items-center justify-center gap-0 mb-8">
    <div v-for="i in 5" :key="i" class="flex items-center gap-0">
      ...
      <div v-if="i < 5" class="w-8 h-0.5 mx-1" :class="i < currentStep ? 'bg-blue-600' : 'bg-slate-200'"></div>
    </div>
  </div>
</template>
...
const labels = ['场景信息', '数据源配置', '数据转换/处理', '输出定义', '预览与导出']
</script>
