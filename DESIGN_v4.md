...
MVP 不使用独立的 PluginFactory 类，引擎直接完成：

```python
# 引擎内部伪代码
plugin_cls = PluginRegistry.get(config.plugin, "input")
plugin = plugin_cls()
plugin.name = config.plugin        # 插件类型名
plugin.label = config.name         # 人类可读名称（仅 InputPlugin 特有）
if isinstance(plugin, InputPlugin):
    plugin.table_name = config.table  # 输入插件特有属性
...
引擎处理步骤：
1. 加载 YAML 中的 `config` → 解析为 Pydantic 模型（此时 `file` 字段缺失或为占位符）
2. 从 `context.params[param_key]` 获取文件路径
3. 将文件路径注入到 `config.file`
4. 调用 `plugin.execute(context, config)`

具体实现有两种方案，引擎任选其一：

- **方案 A（占位符）**：YAML 中写 `file: "__param:person_file__"`，Pydantic 校验前引擎替换
- **方案 B（可选字段）**：Pydantic 模型中 `file: str = ""`，引擎在 `execute` 前赋值

MVP 采用 **方案 B**：`file` 字段默认为空字符串，标记为可选（`str | None = None`），引擎校验 config 后、调用 execute 前注入实际值。

#### 数据读取规范

Input 插件使用 `read_excel_rows(file, sheet)` 逐行读取 Excel：

- **第一次迭代（首行）**：返回列名列表 `["姓名", "部门", "岗位", ...]`，引擎据此在 SQLite 中 `CREATE TABLE`
- **后续迭代（第 2 行起）**：返回数据元组 `("张三", "技术部", "工程师", ...)`，通过 `INSERT` 写入
...
| **输出错误** | 模板文件不存在、目标列在查询结果中缺失、source_table 在数据库中不存在（可能因为 processor SQL 实际创建的表名与 output_tables 声明不一致）、磁盘空间不足 | 终止执行，保留临时 .db 用于调试 |
...
```
阶段 1 (read_only):  加载模板 → 读取目标 Sheet 首行（表头）→ 提取每个单元格的样式对象
...
具体实现：
1. `load_workbook(filename, read_only=True)` 加载模板
2. `iter_rows(min_row=1, max_row=1)` 读取表头行，保存每个单元格的 `font`、`fill`、`border`、`alignment`、`number_format`
...
| Output | Excel（两阶段写入：保留表头样式 + 列宽 + 冻结窗格，按显式列映射输出） |
...
| 21 | read_excel_rows 首行返回列名建表，后续行返回数据 | 确保 SQLite schema 正确创建，不将表头当数据插入 |
...
| 6 | Excel 输入插件（openpyxl read_only，首行建表，逐行插入） | `plugins/input/excel.py` |
