...
> 基于: DESIGN_v7.md (高阶设计 v0.7 终版)  
> 文档版本: v0.2  
> 日期: 2026-05-04  
> 状态: 详细设计阶段  
> 粒度: 模块级伪代码  
> 变更: extra 策略改为 forbid，移除未知字段检测代码，补充错误场景
...
| 2 | `extra="ignore"` 静默丢弃拼写错误 | §4.7 | 改为 `extra="forbid"`，拼写错误当场报错。砍掉 ~40 行未知字段检测代码 |
...
class SceneMeta(BaseModel):
    ...
    model_config = ConfigDict(extra="forbid")    # 向前兼容
...
def load_yaml_config(yaml_path: str) -> SceneConfig:
    ...
    3. SceneConfig.model_validate() 校验（extra="forbid"，未知字段报错）
    4. 执行业务校验（表名冲突检测等）
    """
    ...
    # 步骤 3: Pydantic 校验（extra="forbid" 自动拒绝未知字段）
    try:
...
由于 Pydantic 模型使用 `extra="forbid"`，YAML 中的拼写错误和未知字段在 `model_validate()` 时由 Pydantic 自动拦截，直接抛出 ValidationError，无需手动检测代码。

v0.2 若需向前兼容（允许旧配置忽略新增字段），将关键模型的 `extra` 改为 `"ignore"` 并配合字段默认值即可。
```
...
```
    def load_config(self) -> None:
        ...
        步骤:
        1. load_yaml_config() — YAML 解析 + Pydantic 校验（extra="forbid" 自动检测未知字段）
...
| | `TestExtraForbid` | 未知字段被 Pydantic 拒绝 |
...
| 配置中的未知字段 | ConfigError（extra="forbid"） |
| columns 为空列表 | ConfigError |
| param_key 重复 | ConfigError |
...
| 空 inputs（无输入） | 合法执行，processor 自行建表 |
| 查询结果 0 行 | 输出仅含表头的 Excel，不报错 |
...
| §4.7 | extra="ignore" 忽略未知字段 | 改为 extra="forbid"，拼写错误当场报错 |
| §6.1 | 两阶段写入 | 改为三阶段写入 |
