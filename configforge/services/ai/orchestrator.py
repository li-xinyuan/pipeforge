...
GUIDE_SYSTEM_PROMPT = (
    ...
    "- message 字段中可以包含换行（\\n）和列表\n"
    "- actions 按钮每个步骤至少 1 个\n"
    "- 只聚焦当前步骤，不要提其他步骤的内容\n"
    "- actions 的 value 只能使用以下预定义动作之一，不能自己编造：\n"
    "  第 1 步：confirm（确认并进入下一步）、edit（修改场景信息）\n"
    "  第 2 步：excel / csv / database（选择输入类型）、confirm（输入源配置完成，进入下一步）\n"
    "  第 3 步：pick_sql / pick_python（选择处理器类型）、confirm（代码确认，进入下一步）、suggest_checkpoints / skip_checkpoints（检查点建议/跳过）\n"
    "  第 4 步：confirm（确认输出配置）、keep_columns / update_column_mapping（列映射变更）\n"
    "  第 5 步：download_yaml（下载 YAML 文件）、execute_pipeline（执行流水线）、edit（返回修改）\n"
    "- label 可以根据场景自定义中文文字，但 value 必须是上述预定义值\n"
...
SYSTEM_PROMPTS = {
    ...
    "precheck": (
        ...
        "## 重要说明\n"
        "- 处理步骤的 inputTables 字段可以为空，执行引擎会自动从 SQL 中推断引用的表名，因此 inputTables 为空不是错误，不要报告。\n"
        "- 只有当 SQL 中引用的表名在输入源列表中完全不存在时，才应报告为 error。\n\n"
        "## 返回格式\n"
